...
import RegionalShopDetail from './RegionalDeliveryShopDetail.component';

import { selectedDeliveryCategory, userLocation } from '../../../store/store';
...
const RegionalDeliveryShopsPage = () => {
  ...
  /* 배달 음식점 디테일 페이지 띄움*/
  const handleOpenDeliveryDetail = (event: MouseEvent) => {
    setDetailViewHeight(cardHeightRef.current.clientHeight);
    setSelectDeliveryShop(event.currentTarget.id);
    handleToggleDetail();
  };
            ...
            {limitNumOfItems(deliveryShopList).map((item, idx) => (
              ...
              >
                ...
                <ShopDescContainer id={item.restaurant_id}>
                  ...
                  <p>배달 소요시간</p>: {item.estimated_delivery_time}
                </ShopDescContainer>
              </ShopContainer>
            ))}
          </ShopListContainer>
...
const DeliveryShopListTitle = styled.h1`
  text-align: center;
...
const ShopListContainer = styled.div<Props>`
  display: grid;
  margin: 30px 80px;
  ${({ layout }: Props) =>
    layout === 1
...
const ShopImgContainer = styled.div<Props>`
  width: 100%;
  height: 100%;
